...
module.exports = class Gram {
  ...
  static async popularPosts() {
    const { rows } = await pool.query(`
      SELECT grams.id, COUNT(comments.comment) AS comment FROM grams
        JOIN comments ON post_id = grams.id
        GROUP BY grams.id
    ...
    return rows.map(post => new Gram(post));
  }
};
